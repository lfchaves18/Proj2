...
namespace Proj2
{
    class Escola
    {
        ...
        public List<Professor> ListaProfessorEspera { get; set; } = new List<Professor>();
        public List<Aluno> ListaAlunoEspera { get; set; } = new List<Aluno>();
        ...
        public void RemoverCoordenadorTurma()
        {
            ...
            Console.WriteLine("Em qual turma deseja remover o(a) coordenador(a): "); // pede para informar a turma que deseja remover 
            uint opcaoTurma;
            while (!uint.TryParse(Console.ReadLine(), out opcaoTurma))      // valida se as informações passadas são as desejadas                  
                Console.WriteLine($"Invalido! Em qual turma deseja cadastrar o(a) coordenador(a): "); // se não forem validas, vai ser pedido novamente


            Turma t = ListaTurma.Find(buscaTurma => buscaTurma.NumeroTurma == opcaoTurma); // operação para buscar o numero da turma

            t.CoordNaTurma = null; // remove o coordenador da turma desejada
        }
    }
}
